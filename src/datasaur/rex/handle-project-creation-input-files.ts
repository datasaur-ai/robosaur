...
import {
  AwsS3StorageClient,
  EXPIRED_IN_SECONDS,
} from "../../utils/object-storage/aws-s3-storage";
...
class ProjectCreationInputFilesHandler {
  ...
  public async handle(): Promise<void> {
    for (
      this.currentPage = 0;
      this.currentPage < this.totalDocumentPage();
      this.currentPage++
    ) {
  ...
  private async recognizeDocument(): Promise<DocumentRecognitionResponseData> {
    ...
    try {
      ...
      const response = await axios.post<DocumentRecognitionResponseData>(
        apiUrl,
        form,
        {
  ...
  private filterDocumentFile(data: DocumentRecognitionResponseData): void {
    ...
    for (let i = 0; i < totalPages; i++) {
      if (documentPredictions[i] !== "SURAT_INSTRUKSI") {
        this.deleteFile();
        this.logger.info(
          "The document is not instruction letter. Downloaded file has been deleted."
        );
        return;
      } else {
        this.SICount += 1;
      ...
      if (orientationPredictions[i] !== 0 && imagesData[i] !== null) {
        ...
        this.logger.info(
          "The document is instruction letter and orientation is wrong. Corrected file has been kept."
        );
        return;
      }
    }
  ...
  private async getObjectUrl(
    ...
    return AwsS3StorageClient.getClient(this.getS3Config()).getSignedUrlPromise(
      "getObject",
      {
        Bucket: bucketName,
        Key: objectName,
...
export async function handleProjectCreationInputFiles(
  data: Team15
): Promise<void> {
