...
export class AwsS3StorageClient implements ObjectStorageClient {
  ...
  async listSubfoldersOfPrefix(bucketName: string, rootPrefix = "") {
    const bucketItems: BucketItem[] = await this.listKeysInBucket(
      bucketName,
      rootPrefix
    );
    return bucketItems
      .filter((item: BucketItem) => item.prefix)
  ...
  async getStringFileContent(
    bucketName: string,
    objectPath: string
  ): Promise<string> {
    const request = await AwsS3StorageClient.getClient().getObject({
      Bucket: bucketName,
      Key: objectPath,
    });
  ...
  async setStringFileContent(
    ...
    await AwsS3StorageClient.getClient()
      .upload({ Bucket: bucketName, Key: objectName, Body: content }, options)
      .promise();
  }
