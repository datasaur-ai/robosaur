...
export const pcwTransformer = async (input: Config) => {
  ...
  if (!pcwPayloadSource) {
    ...
    throw new Error('usePcw is true but pcwPayloadSource is not provided, please provide pcwPayloadSource');
  }
  if (!pcwPayload) {
    getLogger().error('usePcw is true but pcwPayload is not provided, please provide pcwPayload');
    throw new Error('usePcw is true but pcwPayload is not provided, please provide pcwPayload');
  }
  ...
  if (pcwPayloadSource?.source === StorageSources.INLINE) {
    if (typeof pcwPayload === 'string') {
      ...
      throw new Error('pcwPayloadSource is "inline" but pcwPayload is given a string');
    }
    ...
    if (typeof pcwPayload !== 'string') {
      ...
      throw new Error(`pcwPayloadSource is ${pcwPayloadSource?.source} but pcwPayload is not given a string`);
    }
